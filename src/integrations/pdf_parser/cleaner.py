...
class TextCleanerMixin:
    ...
    def _normalize_layout_heading_like_text(text: str) -> str:
        ...
        if (
            len(tokens) >= 3
            and re.fullmatch("[A-Z](?:\\.\\d+)*", tokens[0])
            and re.fullmatch("[A-Z]", tokens[1])
            and re.fullmatch("[A-Z]{2,}", tokens[2])
        ):
            prefix = tokens[0]
        ...
        while previous != normalized:
            previous = normalized
            normalized = re.sub(
                "\\b([A-Z])\\s+([A-Z]{2,})\\b", lambda match: f"{match.group(1)}{match.group(2)}", normalized
            )
            normalized = re.sub(
                "\\b([A-Z])\\s+([A-Z][a-z][A-Za-z-]*)\\b",
    ...
    def _prettify_section_title(cls, title: str) -> str:
        ...
        if compact.isupper():
            return re.sub(
                ...
                lambda match: (
                    match.group(0)
                    if match.group(0) in cls._KNOWN_UPPERCASE_TITLE_TOKENS or len(match.group(0)) == 1
    ...
    def _starts_like_body_paragraph(first_line: str, compact: str) -> bool:
        ...
        if not normalized_first_line:
            return False
        if re.match(
            "^(?:As shown|Beyond|In this|In our|We |To address|To evaluate|To further|Notably|While|Since|Early|Figure \\d+ (?:shows|illustrates|visualizes)|Table \\d+ (?:presents|shows|compares|reports|summarizes))\\b",
    ...
    def _looks_like_reference_tail_paragraph(text: str) -> bool:
        ...
        venue_markers = len(
            re.findall(
                "\\b(?:Proceedings|Conference|CVPR|ICCV|ECCV|NeurIPS|ICLR|ACL|EMNLP|AAAI|Google)\\b",
                compact,
                re.IGNORECASE,
            )
        )
        author_list_like = bool(
            re.match(
                "^(?:\\[\\d+\\]\\s*)?[A-Z][A-Za-z'`.-]+,\\s+[A-Z](?:\\.[A-Z])?(?:,\\s+[A-Z][A-Za-z'`.-]+,\\s+[A-Z](?:\\.[A-Z])?){1,}",
                compact,
            )
        )
        ...
        if author_list_like and year_markers >= 1:
            return True
        if compact.startswith(("In: ", "[", "doi:", "https://", "http://")) and (
            year_markers >= 1 or venue_markers >= 1
        ):
            return True
        return False
    ...
    def _normalize_section_heading(cls, line: str) -> str | None:
        ...
        appendix_match = re.fullmatch(
            "(appendix(?:\\s+[A-Z])?)\\s+([A-Z][A-Za-z0-9 ,:/()'&-]{1,100})", candidate, re.IGNORECASE
        )
        if appendix_match:
    ...
    def _normalize_section_text(cls, title: str, text: str) -> str:
        ...
        if cls._infer_content_role(title) == "body":
            lead_fragment_match = re.match(
                "^(?P<lead>[a-z][^.]{0,160}\\.)\\s+(?P<rest>(?:In this section|Here, we|We |This section|To )\\b.+)$",
                compact,
            )
