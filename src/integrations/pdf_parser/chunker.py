...
class SemanticChunkerMixin:
    ...
    @classmethod
    def build_chunks(
        cls, text: str, *, sections: list[dict[str, Any]] | None = None, max_chars: int = 1800, overlap_chars: int = 200
    ) -> list[dict[str, Any]]:
            ...
            while start < len(section_text):
                ...
                if clean_chunk:
                    metadata = {
                        ...
                        "starts_mid_sentence": cls._starts_mid_sentence(section_text, start),
                        "ends_mid_sentence": cls._ends_mid_sentence(section_text, end),
                        "content_role": cls._infer_chunk_content_role(section_title, clean_chunk),
                    }
                    new_chunk = {
                        "chunk_index": chunk_index,
                        "chunk_text": clean_chunk,
                        "section_title": section_title,
                        "token_count": cls._rough_token_count(clean_chunk),
                        "metadata": metadata,
                    }
    ...
    def _should_absorb_into_previous(chunks: list[dict[str, Any]], new_chunk: dict[str, Any]) -> bool:
        ...
        if metadata.get("starts_mid_sentence"):
            return True
        return (
            bool(re.fullmatch("[\\d\\s.,;:()[\\]{}%-]{1,360}", new_text))
            or len(new_text.split()) <= 12
            or bool(re.match("^[a-z0-9]", new_text))
        )
    ...
    def _infer_chunk_content_role(cls, section_title: str, chunk_text: str) -> str:
        ...
        if re.match(
            "^(?:Figure|Table)\\s+\\d+\\s+(?:presents|shows|compares|illustrates|visualizes|reports|summarizes|demonstrates|plots)\\b",
            compact,
            re.IGNORECASE,
        ):
            return "body"
    ...
    def _refine_chunk_content_roles(cls, chunks: list[dict[str, Any]]) -> None:
        for chunk in chunks:
            ...
            if current_role in {"body", "appendix"} and reference_like and (not body_like):
                ...
                continue
            if (
                current_role in {"body", "front_matter"}
                and (not body_like)
                and cls._looks_like_table_like_chunk(chunk_text, compact)
            ):
    ...
    def _looks_like_reference_chunk(cls, compact: str) -> bool:
        ...
        if compact_prefix.startswith("In ") and re.search(
            "\\b(?:Conference|Proceedings|arXiv|Association for Computational Linguistics)\\b", compact_prefix
        ):
            return True
        if (
            re.match("^\\d", compact_prefix)
            and re.search("\\[\\d+\\]", compact_prefix)
            and re.search("\\b(?:19|20)\\d{2}\\b", compact_prefix)
        ):
            return True
        if compact_prefix.startswith(("doi:", "https://", "http://")) and re.search("\\[\\d+\\]", compact_prefix):
            return True
        if re.match("^[A-Z][A-Za-z'`.-]+(?:,\\s+[A-Z][A-Za-z'`.-]+){1,}", compact_prefix) and re.search(
            "\\b(?:19|20)\\d{2}\\b", compact_prefix
        ):
            return True
    ...
    def _looks_like_table_like_chunk(cls, raw_text: str, compact: str) -> bool:
        ...
        short_code_cells = len(
            re.findall("\\b(?:MCQ|OE|A&M|IoU|FVD|PSNR|SSIM|M|A|S|T|R|F1|Top-1|Top-5)\\b", compact, re.IGNORECASE)
        )
        ...
        if re.match("^[\\d.,;:()%-]", compact) and numeric_cells >= 10 and (line_break_count >= 2):
            return True
        if (
            re.search("\\b(?:Table|Figure)\\s+\\d+\\b", compact[:160])
        ...
        if (
            len(lines) >= 8
            and numeric_heavy_lines >= max(6, int(len(lines) * 0.7))
            and (digits / max(1, len(compact)) > 0.08)
        ):
            return True
    ...
    def _looks_like_fragmentary_chunk_start(text: str) -> bool:
        compact = text.lstrip()
        return bool(
            re.match("^\\d{4}[a-z]?\\](?:,)?\\s+[a-z]", compact)
            or re.match("^\\(\\d+\\)\\s+[a-z]", compact)
            or re.match("^\\d+[a-z]?\\),\\s+[a-z]", compact)
            or re.match("^\\d+[a-z]\\),", compact)
            or re.match("^\\d+[a-z]?,\\s+[a-z]", compact)
            or re.match("^\\d+[A-Z][a-z]", compact)
        )
